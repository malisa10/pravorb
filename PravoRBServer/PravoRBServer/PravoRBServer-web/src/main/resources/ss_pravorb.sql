...
INSERT INTO `parts` (`id`, `parent`, `name`) VALUES
	...
	(27, 10, 'Иные правовые акты Республики Беларусь, являющиеся нормативными');
...
BEGIN
	...
	IF buf=0 
		THEN
			return buf;
		ELSE
			SELECT `id` INTO buf FROM `users` WHERE `users`.`login`=login AND `users`.`password`=MD5(MD5(password));
			return buf;
		END IF;
END//
DELIMITER ;
...
BEGIN
	UPDATE `users` SET `password`=MD5(MD5(password)) WHERE `id`=id;
END//
DELIMITER ;
...
BEGIN
	...
	SELECT `password` INTO buf FROM `users` WHERE `id`=id;
	IF MD5(MD5(oldpassword))=buf
		THEN
			UPDATE `users` SET `password`=MD5(MD5(newpassword)) WHERE `id`=id;
		END IF;
END//
DELIMITER ;
...
BEGIN
	INSERT INTO `users` (`login`, `password`) VALUES (login, MD5(MD5(password)));
END//
DELIMITER ;
...
BEGIN
	UPDATE `users` SET `group`=groupid WHERE `id`=id;
END//
DELIMITER ;
...
INSERT INTO `users` (`id`, `login`, `password`, `firstname`, `lastname`, `email`, `address`, `zipcode`, `telephone`, `group`) VALUES
	(1, 'admin', 'fb469d7ef430b0baf0cab6c436e70375', 'Марина', 'Андреенко', 'test@test.by', '', '', '+375-29-897-82-52', 1),
	(4, 'moderator', 'fb469d7ef430b0baf0cab6c436e70375', 'Тест 1', 'Тест 1', 'test1@test1.by', '', '', '', 2),
	(6, 'user', 'fb469d7ef430b0baf0cab6c436e70375', 'Тест 2', 'Тест 2', 'test2@test2.by', '', '', '', 3);
